...
Each report passes through an 11-step pipeline: keyword-based intent pre-filter → LLM zero-shot structured extraction (JSON schema: domain, hazard type, location, description, people-at-risk, ongoing, scale) → mandatory single-turn location clarification if missing → Open-Meteo rainfall lookup for plausibility → composite confidence scoring (completeness 0.30, plausibility 0.20, Bayesian triangulation 0.30, photographic evidence 0.20 — §3.4.1) → severity scoring (exposure 0.40, infrastructure impact 0.20, hazard scale 0.30, immediacy 0.10) → a confidence×severity decision matrix (`store_only` / `monitor` / `flag_review` / `escalate`) → persistence with a full audit log → reporter-reliability update. The knowledge base and the community-report store are architecturally separate; community observations only ever enter the advisory context as explicitly labelled, unverified supplementary text.

#### 3.4.1 Multimodal extension: photographic evidence scoring

Citizens may attach a photo to a WhatsApp report. The image is retrieved via the WhatsApp Cloud API's two-step media endpoint, persisted to local storage for audit, and passed — alongside the citizen's text — to the same GPT-4o-mini model used elsewhere in the pipeline, now invoked in multimodal mode. The model is prompted against a fixed five-tier evidence rubric (0.00 no/unusable image; 0.05 image does not clearly support the claim; 0.10 partially consistent evidence; 0.15 clear supporting evidence; 0.20 strong supporting evidence) and simultaneously returns a structured scene assessment: `hazard_detected`, `hazard_category` (drainage issue, slope instability, ground cracking, localised flooding, or access obstruction), `severity_from_image`, and text–image consistency. When the accompanying caption is too sparse to classify the report on its own — an observed failure mode for photo-first submissions where the caption is only a place name — the photo's own `hazard_category` and `severity_from_image` deterministically backfill `report_domain`, `hazard_type`, and `hazard_scale`, and the detected hazard is surfaced back to the citizen in the (trilingual) acknowledgement message. This closes a text-classification blind spot rather than merely adding an independent scoring signal (§5.5.1).
...
| Vision-LLM photographic evidence scoring (multimodal GPT-4o-mini, §3.4.1) | Real code, real OpenAI calls | Newly added capability; validated only against a small ad hoc hand-labelled photo pilot (§5.5.1), not yet part of the reproducible `evaluation/` suite — see §7. |
...
#### 5.5.1 Photographic evidence: a preliminary pilot

The multimodal evidence-scoring extension (§3.4.1) was checked, post-deployment, against six real hazard photographs (drainage flooding ×2, slope instability, ground cracking, road flooding, fallen tree) each carrying an independent reference evidence score produced under the same five-tier rubric.

**Table 6a.** Photo evidence-score agreement against independently reference-scored photographs (n=6; one image had no independent reference and is excluded from the match statistic).

| Metric | Value |
|---|---|
| Exact tier match | 3/5 (60%) |
| Maximum observed deviation | 1 tier (0.05 of a 0.00–0.20 scale) |
| Direction of all deviations | Conservative — scored *below* the reference tier, never above; no false positives observed |
| Hazard/domain backfill case study | A photo-plus-location-only caption (no hazard description) that previously scored confidence 0.25 (`store_only`) scored confidence 0.60 (`monitor`) once the vision model's own hazard classification was permitted to backfill the missing text-derived fields |

This is a feasibility check, not a validated benchmark: n=6 is far smaller than any other test set in this paper, the reference scores were not independently multi-rater-verified, and the pilot is not yet wired into the reproducible `evaluation/` benchmark suite (§7, §8).
